...
function parseRedisUrl(url: string): RedisOptions {
  ...
  if (parsed.protocol !== "redis:" && parsed.protocol !== "rediss:") {
    throw new Error(
      `[redis] Unsupported URL protocol "${parsed.protocol}". Use redis:// or rediss://`,
    );
  }
...
export function getRedisConnectionOptions(): RedisOptions {
  if (redisDisabledForRuntime) {
    throw new Error(
      "[redis] Disabled in web-only runtime mode. Set FYNT_RUNTIME_MODE=full to enable Redis.",
    );
  }
  ...
  if (redisUrl) {
    const configuredFrom = process.env.REDIS_URL?.trim()
      ? "REDIS_URL"
      : "UPSTASH_REDIS_URL";
    try {
      ...
      throw new Error(
        `[redis] Invalid ${configuredFrom}. Expected redis:// or rediss:// URL. ${details}`,
      );
    }
  }
...
export function isRedisConnectionError(error: unknown): boolean {
  ...
  if (
    ...
    nestedErrors.some((nestedError) => isRedisConnectionError(nestedError))
  ) {
    return true;
  }
...
function attachRedisErrorLogging(client: Redis, label: string): void {
  client.on("error", (error) => {
    ...
    logThrottledWarning(
      redisErrorLogTimestamps,
      `${label}:${reason}`,
      `[redis:${label}] unavailable: ${reason}`,
    );
  });
}
...
export async function withRedisFallback<T>(
  context: string,
  operation: () => Promise<T>,
  fallback: RedisFallbackFactory<T>,
): Promise<T> {
  if (redisDisabledForRuntime) {
    logThrottledWarning(
      fallbackLogTimestamps,
      `${context}:runtime-disabled`,
      `[redis-fallback:${context}] disabled in web-only runtime mode`,
    );
