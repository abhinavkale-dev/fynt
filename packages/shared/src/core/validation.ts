...
function validateProviderJsonInput(
  nodeType: string,
  requestJson: string,
): string | null {
  const trimmed = requestJson.trim();
  if (!trimmed) return null;
  if (hasTemplateVariableSyntax(trimmed)) return null;
  let parsed: unknown;
  try {
...
function getReachableNodeIds(
  ...
  const triggers = nodes.filter(
    (node) => TRIGGER_TYPES.includes(node.type) && isNodeActive(node),
  );
...
export function checkMissingConfigurations(
  ...
  for (const node of nodes) {
    ...
    const issueType: ValidationIssue["type"] = isReachable
      ? "error"
      : "warning";
    ...
    switch (type) {
      ...
      case "geminiNode": {
        ...
        if (!data.credentialId?.trim()) missingFields.push("credential");
        if (!data.model?.trim()) missingFields.push("model");
        if (inputMode === "json") {
          if (!data.requestJson?.trim()) missingFields.push("JSON input");
      ...
      case "githubNode": {
        const missingFields: string[] = [];
        const operation =
          typeof data.operation === "string" ? data.operation : "create_issue";
        if (!data.credentialId?.trim()) missingFields.push("credential");
        if (!data.owner?.trim()) missingFields.push("owner");
        if (!data.repo?.trim()) missingFields.push("repo");
        if (operation === "create_issue") {
          if (!data.title?.trim()) missingFields.push("title");
        } else if (operation === "create_comment") {
          if (!data.issueNumber?.trim()) missingFields.push("issueNumber");
          if (!data.body?.trim()) missingFields.push("body");
        ...
        if (missingFields.length > 0) {
          pushIssue(
            `${label}: Missing ${missingFields.join(", ")}`,
            "Configure GitHub credential, operation, and required fields.",
          );
        }
        break;
      }
      case "notionNode": {
        const missingFields: string[] = [];
        const operation =
          typeof data.operation === "string" ? data.operation : "create";
        const dataSourceRaw =
          typeof data.dataSourceId === "string" ? data.dataSourceId.trim() : "";
        const databaseRaw =
          typeof data.databaseId === "string" ? data.databaseId.trim() : "";
        const pageRaw =
        ...
        if (!data.credentialId?.trim()) missingFields.push("credential");
        if (operation === "create") {
          if (!hasTargetId) missingFields.push("dataSourceId or databaseId");
        ...
        if (missingFields.length > 0) {
          pushIssue(
            `${label}: Missing ${missingFields.join(", ")}`,
            "Configure Notion credential, operation, and required fields.",
          );
        }
        break;
      }
      ...
      case "conditionNode":
        if (
          ...
          pushIssue(
            `${label}: Missing expression or routes`,
            "Configure the expression and at least one route",
          );
        }
        break;
      case "filterNode":
        if (!data.expression?.trim()) {
          pushIssue(
            `${label}: Missing filter expression`,
            "Configure the filter expression",
          );
        }
        break;
      case "transformNode":
        if (!data.expression?.trim()) {
          pushIssue(
            `${label}: Missing transform expression`,
            "Configure the transform expression",
          );
        }
        break;
      case "delayNode":
        if (!data.durationMs || data.durationMs <= 0) {
          pushIssue(
            `${label}: Invalid delay duration`,
            "Set a positive delay duration (in milliseconds)",
          );
        }
        break;
      case "slackNode":
...
export function validateWorkflow(
  ...
  const triggerNodes = nodes.filter((node) =>
    TRIGGER_TYPES.includes(node.type),
  );
  ...
  if (triggerNodes.length > 0 && activeTriggerNodes.length === 0) {
    issues.push({
      ...
      suggestion:
        "Activate at least one trigger node before running the workflow.",
    });
  }
  const circularIssue = detectCircularDependencies(nodes, edges);
  if (circularIssue) issues.push(circularIssue);
  const orphanedIssue = findOrphanedNodes(nodes, edges);
  if (orphanedIssue) issues.push(orphanedIssue);
