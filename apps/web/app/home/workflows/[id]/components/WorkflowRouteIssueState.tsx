...
interface WorkflowRouteIssueStateProps {
    title: string;
    description: string;
    code: string;
    onRetry?: () => void;
...
export function WorkflowRouteIssueState({ title, description, code, onRetry, retryLabel = 'Retry', showNavigationLinks = true, }: WorkflowRouteIssueStateProps) {
    return (<div className="flex h-full min-h-0 items-center justify-center bg-[#151515] px-6 py-10">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.02),rgba(255,255,255,0.005))] p-8 text-center shadow-[0_20px_40px_-12px_rgba(0,0,0,0.45)]">
        ...
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-white/50">{code}</p>
        <h1 className="mt-2 text-3xl font-bold text-white">{title}</h1>
        <p className="mt-3 text-sm text-white/60">{description}</p>

        {(onRetry || showNavigationLinks) && (<div className="mt-8 flex flex-wrap items-center justify-center gap-2">
            {onRetry && (<button type="button" onClick={onRetry} className="inline-flex h-10 items-center justify-center rounded-xl border border-white/15 bg-white/5 px-4 text-sm font-medium text-white transition-colors hover:bg-white/10">
                {retryLabel}
              </button>)}

            {showNavigationLinks && (<>
                <Link href="/home" className="inline-flex h-10 items-center justify-center rounded-xl border border-white/15 bg-white/5 px-4 text-sm font-medium text-white transition-colors hover:bg-white/10">
                  Back to Workflows
                </Link>
                <Link href="/home/templates" className="inline-flex h-10 items-center justify-center rounded-xl bg-[#F04D26] px-4 text-sm font-semibold text-white transition-colors hover:bg-[#e04420]">
                  Browse Templates
                </Link>
              </>)}
          </div>)}
