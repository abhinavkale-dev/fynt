...
type AutoCredentialByProvider = Partial<
  Record<"openai" | "anthropic" | "gemini", string>
>;
...
function getAIProviderForNodeType(
  nodeType: string,
): keyof AutoCredentialByProvider | null {
  if (nodeType === "openaiNode" || nodeType === "aiNode") return "openai";
...
export function useWorkflowCanvasActions({
...
}: UseWorkflowCanvasActionsParams) {
  const onConnect = useCallback(
    (params: Connection | Edge) => {
      onBeforeGraphMutation?.();
      const routeLabel =
        "sourceHandle" in params && typeof params.sourceHandle === "string"
          ? params.sourceHandle
          : undefined;
    ...
    (nodeType: string, label: string) => {
      ...
      const newNode: Node = {
        ...
        data: {
          ...
          nodeType,
          isConfigured: false,
          ...(prefilledCredentialId
            ? { credentialId: prefilledCredentialId }
            : {}),
        },
      };
      onBeforeGraphMutation?.();
  ...
  const updateNodeData = useCallback(
    async (nodeId: string, data: unknown) => {
      const incomingData =
      ...
      const updatedNodes = nodes.map((node: Node) => {
        ...
        const hasCredential =
          typeof mergedData.credentialId === "string" &&
          mergedData.credentialId.trim().length > 0;
        if (
          hasCredential &&
          (node.type === "discordNode" || node.type === "slackNode")
        ) {
  ...
  const addNodeAtPosition = useCallback(
    (type: string, position = { x: 250, y: 50 }) => {
