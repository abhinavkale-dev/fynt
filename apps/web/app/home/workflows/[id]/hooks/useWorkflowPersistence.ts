...
function parseGraphArray<T>(value: unknown): T[] {
  ...
  if (value && typeof value === "object") {
    ...
    if (
      entries.length > 0 &&
      entries.every(([key]) => Number.isInteger(Number(key)))
    ) {
      return entries
        .sort((a, b) => Number(a[0]) - Number(b[0]))
        .map(([, item]) => item as T);
...
export function useWorkflowPersistence({
  ...
  const enqueueWorkflowUpdateRef = useRef<
    (
      workflowId: string,
      snapshot: WorkflowSaveSnapshot,
      saveVersion: number,
  ...
  >(async () => {});
  const lastValidGraphRef = useRef<Pick<
    WorkflowSaveSnapshot,
    "nodes" | "edges"
  > | null>(null);
  ...
  const [graphRecoveryNotice, setGraphRecoveryNotice] = useState<string | null>(
    null,
  );
  const [graphIntegrityError, setGraphIntegrityError] = useState<string | null>(
    null,
  );
  ...
  const debouncedChangeVersion = useDebounce(
    changeVersion,
  ...
  const isTemplateNodeCredentialValid = useCallback(
  ...
  const enqueueWorkflowUpdate = useCallback(
    async (
      ...
      const validation = validateGraphShape(
        queuedSnapshot.nodes as Array<{
          id?: unknown;
        }>,
      ...
      const isTemplateGraphEmpty = Boolean(
        template && queuedSnapshot.nodes.length === 0,
      );
      if (!validation.isValid || isTemplateGraphEmpty) {
        ...
        if (fallbackGraph) {
          ...
          setSaveStatus("error");
          setLastSaveError(
            `Invalid workflow graph snapshot was blocked. Reverted to last valid graph. ${validationErrorMessage}`,
          );
        } else {
          setSaveStatus("error");
          setLastSaveError(
            `Invalid workflow graph snapshot was blocked. ${validationErrorMessage}`,
          );
          return;
        }
      }
      ...
      const runSave = () => {
        if (workflowId === "new") return Promise.resolve();
      ...
      saveQueueRef.current = saveQueueRef.current.catch(() => {}).then(runSave);
    },
  ...
  const handleAutoSave = useCallback(async () => {
    ...
    if (debouncedChangeVersion !== changeVersionRef.current) return;
    const saveVersion = changeVersionRef.current;
    await enqueueWorkflowUpdate(
      currentWorkflowId,
      {
  ...
  const handleImmediateSave = useCallback(
    async (overrides?: { nodes?: Node[]; edges?: Edge[]; title?: string }) => {
      if (currentWorkflowId === "new") return;
      const saveVersion = changeVersionRef.current;
      await enqueueWorkflowUpdate(
        currentWorkflowId,
  ...
  const handleManualSave = useCallback(async () => {
    try {
      ...
      const graphValidation = validateGraphShape(
        nodes as Array<{
          id?: unknown;
        }>,
        edges as Array<{
      ...
      if (!graphValidation.isValid) {
        throw new Error(
          `Cannot save invalid graph. ${graphValidation.errors.join(" ")}`,
        );
      }
  ...
  useEffect(() => {
    setCurrentWorkflowId((previousWorkflowId) =>
      previousWorkflowId === id ? previousWorkflowId : id,
    );
    if (id === "new") {
      setNodes([]);
      setEdges([]);
      ...
      if (hasCorruptGraph && template) {
        const attemptedRepair = attemptedGraphRepairRef.current.has(
          workflowData.id,
        );
        const repairedGraph = instantiateTemplateGraphPreservingCredentials(
          template,
          workflowNodes,
          undefined,
          getTemplateDefaultFields(template),
        );
        ...
        if (!attemptedRepair) {
          ...
          setChangeVersion(nextVersion);
          setHasUserMadeChanges(true);
          void enqueueWorkflowUpdateRef.current(
            workflowData.id,
            {
        ...
        setGraphRecoveryNotice(null);
        if (!graphValidation.isValid) {
          setGraphIntegrityError(
            `Workflow graph is invalid. ${graphValidation.errors.join(" ")}`,
          );
        } else {
          setGraphIntegrityError(null);
          lastValidGraphRef.current = {
  ...
  useEffect(() => {
    if (!workflowData || !template || template.requiredBindings.length === 0)
      return;
    if (workflowData.id !== id) return;
    if (!credentialsReady) return;
    if (templateSetupDismissed) return;
    const nodeById = new Map(nodes.map((node) => [node.id, node]));
    const hasMissingCredentials = template.requiredBindings.some((binding) =>
    ...
    const hasMissingRequiredFields = (template.fieldRequirements ?? []).some(
      (fieldRequirement) => {
        if (!fieldRequirement.required) return false;
    ...
    template,
    nodes,
    templateSetupDismissed,
    credentialsReady,
    isTemplateNodeCredentialValid,
  ]);
  useEffect(() => {
