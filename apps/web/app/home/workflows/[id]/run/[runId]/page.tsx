...
function formatOutput(output: any): string {
  ...
  if (output._truncated && output._preview) return output._preview;
    ...
    <>
      <Topbar title={run.workflow.title}>
        <div className="flex items-center gap-3">
          <span
            className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border transition-colors duration-200 ease ${STATUS_BG[effectiveStatus] || ""} ${STATUS_COLORS[effectiveStatus] || ""}`}
          >
            {effectiveStatus}
          </span>
          ...
          <Link
            href={`/home/workflows/${workflowId}`}
      ...
      >
        ...
        {showNodesList && mergedNodeRuns.length > 0 && (
          <div className="absolute right-4 top-20 max-w-xs z-10">
            <div className="bg-[#1a1a1a] rounded-lg border border-[#333] p-3 shadow-lg">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-xs font-medium text-white/70">
                  Node Runs ({mergedNodeRuns.length})
                </h3>
                <button
                  onClick={() => setShowNodesList(false)}
              ...
              <div className="space-y-1 max-h-96 overflow-y-auto">
                {mergedNodeRuns.map((nr) => (
                  ...
                  >
                    ...
                    <span
                      className={`${STATUS_COLORS[nr.status] || "text-white/50"} font-medium ml-2`}
                    >
                      {nr.status}
                    </span>
                  </button>
        ...
        <Sheet
          open={selectedNodeRunId !== null}
          onOpenChange={(open) => {
            if (!open) setSelectedNodeRunId(null);
          }}
        >
          <SheetContent
            side="right"
            className="w-96 overflow-y-auto bg-[#141414] border-l border-[#333]"
          >
            <SheetHeader>
            ...
            <div className="mt-6 space-y-2">
              {selectedNodeRunId &&
                (() => {
                  const selectedRun = mergedNodeRuns.find(
                    (nr) => nr.id === selectedNodeRunId,
                  );
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-white/70 font-medium">
                          {selectedRun.nodeType}
                        </span>
                        <span
                          className={`text-xs font-medium transition-colors duration-200 ease ${STATUS_COLORS[selectedRun.status] || "text-white/50"}`}
                        >
                          {selectedRun.status}
                        </span>
                      </div>
                        ...
                        {selectedRun.output &&
                          selectedRun.status === "Success" && (
                            <div className="mt-2">
                              ...
                              {selectedRun.output?._truncated && (
                                <div className="text-[10px] text-white/30 mt-1 italic">
                                  Output truncated (
                                  {Math.round(
                                    selectedRun.output._previewLength / 1024,
                                  )}
                                  KB). Full output loads on completion.
                                </div>
                              )}
                            </div>
                          )}
