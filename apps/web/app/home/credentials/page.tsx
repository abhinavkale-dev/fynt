...
const PLATFORMS = [
  ...
  },
  {
    ...
    description: "Claude models for AI tasks",
  },
  {
    value: "gemini",
    label: "Gemini",
  ...
  },
  {
    ...
    description: "Repository and issue automation",
  },
  {
    value: "notion",
    label: "Notion",
    ...
    case "notion":
      return [
        ...
        },
        {
          ...
          inputType: "text",
          helperText:
            "Paste a shared Notion page or database link. You can reuse this when configuring Notion nodes.",
        },
      ];
    case "slack":
      return [
...
function CredentialsPageContent() {
  ...
  const [managePlatform, setManagePlatform] = useState<PlatformValue | null>(
    null,
  );
  const queryPlatform = searchParams.get("platform");
  const requestedPlatform = isPlatformValue(queryPlatform)
    ? queryPlatform
    : null;
  ...
  const basicKeyFields = useMemo(
    () => keyFields.filter((field) => !field.advanced),
    [keyFields],
  );
  const advancedKeyFields = useMemo(
    () => keyFields.filter((field) => field.advanced),
    [keyFields],
  );
  const missingRequiredBasicFields = useMemo(
    () =>
      editingId
        ? []
        : basicKeyFields
    ...
    [basicKeyFields, editingId, keys],
  );
  const isSaveDisabled =
    isSaving || !title.trim() || missingRequiredBasicFields.length > 0;
  return (
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-amber-100">
              Add the missing credentials, then return to complete template
              setup.
            </p>
            <Button
              size="sm"
              variant="outline"
              onClick={() => router.push(returnToPath as never)}
              className="shrink-0 border-white/30 bg-transparent text-white hover:bg-white/10 hover:text-white"
            >
              Back to Template
            </Button>
          </div>
      ...
      {isLoading ? (
        <div className="grid gap-2.5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {PLATFORMS.map((platform) => (
            <Card
              key={platform.value}
              className="bg-card border border-[#333] overflow-hidden h-fit animate-pulse"
            >
        ...
        <div className="grid gap-2.5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 items-start">
          {PLATFORMS.map((p, index) => {
            const platformCreds = credentialsByPlatform[p.value] ?? [];
            return (
              <Card
                key={p.value}
                className="bg-card border border-[#333] transition-colors animate-in fade-in-0 slide-in-from-bottom-1 duration-200 overflow-hidden h-fit"
                style={{
                ...
                <div className="p-3 flex items-center gap-3">
                  <div className="relative shrink-0">
                    ...
                    {platformCreds.length > 0 && (
                      <span className="absolute -top-1 -right-1 inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-semibold bg-[#f04d26] text-white leading-none">
                  ...
                  <button
                    onClick={() => setManagePlatform(p.value)}
                    className="shrink-0 p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded transition-colors duration-150"
                    title="Manage keys"
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <div className="flex items-center gap-3 mb-1">
              {managePlatform &&
                PLATFORMS.find((p) => p.value === managePlatform) && (
                  <div className="w-8 h-8 rounded-lg bg-[#2D2D2E] flex items-center justify-center p-1.5">
                    <Image
                      src={
                        PLATFORMS.find((p) => p.value === managePlatform)!.logo
                      }
                      alt={
                        PLATFORMS.find((p) => p.value === managePlatform)!.label
                      }
                      width={20}
                      height={20}
                    />
              ...
              <DialogTitle className="text-white">
                Manage{" "}
                {PLATFORMS.find((p) => p.value === managePlatform)?.label} Keys
              </DialogTitle>
            </div>
          ...
          <div className="space-y-3 max-h-[60vh] overflow-y-auto">
            {managePlatform &&
            credentialsByPlatform[managePlatform] &&
            credentialsByPlatform[managePlatform].length > 0 ? (
              <motion.div
                className="space-y-3"
                initial="hidden"
                animate="show"
              ...
              >
                {credentialsByPlatform[managePlatform]!.map((cred) => (
                  <motion.div
                    key={cred.id}
                    variants={{
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 ml-3 shrink-0">
                      ...
                      <button
                        ...
                        className="p-2 text-muted-foreground hover:text-white hover:bg-red-600 rounded transition-colors"
                        title="Delete"
          ...
          <DialogHeader>
            <div className="flex items-center gap-3 mb-1">
              {dialogPlatformMeta && (
                <div className="w-8 h-8 rounded-lg bg-[#2D2D2E] flex items-center justify-center p-1.5">
                  <Image
                    src={dialogPlatformMeta.logo}
                    alt={dialogPlatformMeta.label}
                    width={20}
                    height={20}
                  />
                </div>
              ...
              <DialogTitle>
                {editingId
                  ? `Edit ${dialogPlatformMeta?.label ?? ""} Credential`
                  : `Add ${dialogPlatformMeta?.label ?? ""} Credential`}
              </DialogTitle>
            </div>
            <DialogDescription>
              {editingId
          ...
          >
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder={`My ${dialogPlatformMeta?.label ?? ""} Key`}
                className="bg-[#2D2D2E] border-[#444] text-white placeholder:text-white/30"
              />
            </div>

            {basicKeyFields.map((field) => (
              <div key={field.key} className="space-y-2">
                ...
                />
                {field.helperText && (
                  <p className="text-xs text-white/40">{field.helperText}</p>
                )}
              </div>
            ))}

            {!editingId && missingRequiredBasicFields.length > 0 && (
              <p className="text-xs text-amber-300">
                Required before save: {missingRequiredBasicFields.join(", ")}
              </p>
            )}

            {advancedKeyFields.length > 0 && (
              <div className="space-y-3 rounded-md border border-white/10 bg-white/[0.02] p-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    className="text-xs text-[#F04D26] hover:text-[#ff6a47] transition-colors"
                  >
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure? This action cannot be undone. Any workflow nodes
