...
export async function POST(
  ...
  const { ipPerMinute, endpointPerMinute, bodyMaxBytes } =
    getWebhookRateLimitConfig();
  ...
  try {
    ...
    if (!ipRate.allowed) {
      return NextResponse.json(
        { error: "Webhook rate limit exceeded for this IP." },
        {
          status: 429,
          headers: {
    ...
    return NextResponse.json(
      { error: "Webhook protection service unavailable." },
      { status: 503 },
    );
  }
  ...
  if (!workflow.userId || !workflow.user) {
    return NextResponse.json(
      { error: "Workflow owner not found. Cannot execute webhook run." },
      { status: 400 },
    );
  }
  ...
  const triggerNode = nodes.find(
    (node) => node?.id === nodeId && node?.type === "webhookTrigger",
  );
  if (!triggerNode) {
    return NextResponse.json(
  ...
  if (nodeData.isActive === false) {
    return NextResponse.json(
      { error: "Webhook trigger node is deactivated" },
      { status: 409 },
    );
  }
  if (nodeData.isConfigured !== true) {
    return NextResponse.json(
      { error: "Webhook trigger node is not configured yet" },
      { status: 400 },
    );
  }
  const configuredSecret =
    typeof nodeData.secret === "string" ? nodeData.secret.trim() : "";
  ...
  if (!configuredSecret) {
    return NextResponse.json(
      { error: "Webhook trigger secret is not configured for this node" },
      { status: 400 },
    );
  }
  ...
  const effectivePlan = getEffectivePlan(
    workflow.user.plan,
    workflow.user.paidUntil,
    now,
  );
  const { monthlyRunLimit, maxConcurrent } = getPlanLimits(effectivePlan);
  let workflowRunId = "";
  try {
    const workflowRun = await withUserRunReservationLock(
      workflow.userId as string,
      () =>
        prisma.$transaction(async (tx) => {
          await tx.usageRecord.upsert({
            where: {
              userId_month: { userId: workflow.userId as string, month },
            },
    ...
    if (error instanceof RunReservationLockTimeoutError) {
      return NextResponse.json(
        {
          error:
            "Another execution request is being processed. Please retry shortly.",
        },
        { status: 429 },
      );
    }
    ...
    return NextResponse.json(
      { error: "Failed to reserve workflow run" },
      { status: 500 },
    );
  }
  try {
    ...
    await prisma
      ...
      .catch((rollbackError) => {
        console.error(
          "[webhook] Failed to rollback usage after enqueue error:",
          rollbackError,
        );
      });
    return NextResponse.json(
