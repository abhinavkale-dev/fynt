...
function Sidebar({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }: React.ComponentProps<"div"> & {
    ...
    if (isMobile) {
        ...
        <SheetContent data-sidebar="sidebar" data-slot="sidebar" data-mobile="true" className="bg-sidebar text-sidebar-foreground w-(--sidebar-width) border-0 p-0 [&>button]:hidden" style={{
    ...
    return (<div className="group peer text-sidebar-foreground hidden md:block" data-state={state} data-collapsible={state === "collapsed" ? collapsible : ""} data-variant={variant} data-side={side} data-slot="sidebar">
      ...
      <div data-slot="sidebar-gap" className={cn("relative w-(--sidebar-width) bg-transparent transition-[width] duration-200 ease-out", "group-data-[collapsible=offcanvas]:w-0", "group-data-[side=right]:rotate-180", variant === "floating" || variant === "inset"
            ? "group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4)))]"
            : "group-data-[collapsible=icon]:w-(--sidebar-width-icon)")}/>
      <div data-slot="sidebar-container" className={cn("fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-out md:flex", side === "left"
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
    ...
    return (<Button data-sidebar="trigger" data-slot="sidebar-trigger" variant="ghost" size="icon" className={cn("size-7", className)} onClick={(event) => {
      ...
      <svg height="18" width="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
        <g fill="currentColor">
          <path d="M3.75011 16L9.00009 16L9.00009 2L3.75011 2C2.23351 2 1.00011 3.2334 1.00011 4.75L1.00011 13.25C1.00011 14.7666 2.23351 16 3.75011 16Z" fill="currentColor"/>
          <path d="M14.2501 16C15.7667 16 17.0001 14.7666 17.0001 13.25L17.0001 4.75C17.0001 3.2334 15.7667 2 14.2501 2L9.00009 2L9.00009 16L14.2501 16Z" fill="currentColor" fillOpacity="0.4"/>
          <path d="M14.2501 5.5L11.7501 5.5C11.336 5.5 11.0001 5.8359 11.0001 6.25C11.0001 6.6641 11.336 7 11.7501 7L14.2501 7C14.6642 7 15.0001 6.6641 15.0001 6.25C15.0001 5.8359 14.6642 5.5 14.2501 5.5Z" fill="currentColor"/>
          <path d="M14.2501 8.25L11.7501 8.25C11.336 8.25 11.0001 8.5859 11.0001 9C11.0001 9.4141 11.336 9.75 11.7501 9.75L14.2501 9.75C14.6642 9.75 15.0001 9.4141 15.0001 9C15.0001 8.5859 14.6642 8.25 14.2501 8.25Z" fill="currentColor"/>
          <path d="M14.2501 11L11.7501 11C11.336 11 11.0001 11.3359 11.0001 11.75C11.0001 12.1641 11.336 12.5 11.7501 12.5L14.2501 12.5C14.6642 12.5 15.0001 12.1641 15.0001 11.75C15.0001 11.3359 14.6642 11 14.2501 11Z" fill="currentColor"/>
        </g>
      </svg>
...
function SidebarMenuButton({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, onClick, ...props }: React.ComponentProps<"button"> & {
    asChild?: boolean;
    isActive?: boolean;
    ...
    const button = (<Comp data-slot="sidebar-menu-button" data-sidebar="menu-button" data-size={size} data-active={isActive} className={cn(sidebarMenuButtonVariants({ variant, size }), className)} onClick={(event) => {
            onClick?.(event);
...
function SidebarMenuSubButton({ asChild = false, size = "md", isActive = false, className, onClick, ...props }: React.ComponentProps<"a"> & {
    asChild?: boolean;
    size?: "sm" | "md";
    isActive?: boolean;
}) {
    ...
    return (<Comp data-slot="sidebar-menu-sub-button" data-sidebar="menu-sub-button" data-size={size} data-active={isActive} onClick={(event) => {
            onClick?.(event);
            if (event.defaultPrevented)
                return;
            if (isMobile) {
