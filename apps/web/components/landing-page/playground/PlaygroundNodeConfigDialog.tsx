...
export function PlaygroundNodeConfigDialog({
  open,
  nodeId,
  nodeType,
  nodeData,
  onOpenChange,
  onSave,
}: PlaygroundNodeConfigDialogProps) {
  ...
  const isSupportedType = nodeType
    ? SUPPORTED_NODE_TYPES.has(nodeType as PlaygroundConfigNodeType)
    : false;
  useEffect(() => {
    if (!open) {
      return;
    ...
    const nextLabel =
      typeof nextData.label === "string" && nextData.label.trim().length > 0
        ? nextData.label
        : getDefaultLabel(nodeType);
    setLabel(nextLabel);
    setModel(
      typeof nextData.model === "string" ? nextData.model : "gpt-5-mini",
    );
    setPrompt(
      typeof nextData.prompt === "string"
        ? nextData.prompt
        : "Summarize updates and classify risk level.",
    );
    setDemoRoute(nextData.demoRoute === "log" ? "log" : "alert");
    setSubtitle(
      typeof nextData.subtitle === "string"
        ? nextData.subtitle
        : "Send message",
    );
    setMessage(
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (nodeType === "openaiNode") {
      ...
      payload.prompt =
        prompt.trim() || "Summarize updates and classify risk level.";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        {!isSupportedType ? (
          <div className="rounded-md border border-[#333] bg-[#151515] px-4 py-3 text-sm text-white/60">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder={getDefaultLabel(nodeType)}
              />
            </div>
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    placeholder="Summarize updates and classify risk level."
                  />
                </div>
              </>
            )}
            ...
            {nodeType === "conditionNode" && (
              <div className="space-y-2">
                ...
                <select
                  value={demoRoute}
                  onChange={(event) =>
                    setDemoRoute(event.target.value === "log" ? "log" : "alert")
                  }
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full rounded-md border border-[#444] bg-[#2D2D2E] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-1 focus:ring-ring resize-none"
                    placeholder="High-risk update detected"
                  />
                </div>
              </>
            )}
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full rounded-md border border-[#444] bg-[#2D2D2E] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-1 focus:ring-ring resize-none"
                    placeholder="Low-risk update logged"
                  />
                </div>
              </>
            )}
            ...
            <DialogFooter>
              <Button
                type="button"
                variant="outline"
                className="border-[#444] bg-[#2D2D2E] text-white hover:bg-[#3A3A3A]"
