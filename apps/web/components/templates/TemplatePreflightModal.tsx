...
function isClearlyInvalidNotionId(value: string): boolean {
  ...
  if (/^x{16,}$/i.test(trimmed)) return true;
  if (/^0{16,}$/i.test(trimmed)) return true;
  if (trimmed === "change-this-secret") return true;
  return false;
}
...
export function TemplatePreflightModal({
  ...
  onClose,
  onConfirm,
}: TemplatePreflightModalProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const { data: credentials = [], isLoading } =
    trpc.credentials.getAll.useQuery();
  ...
  const unboundFieldRequirements = useMemo(() => {
    ...
    return template.fieldRequirements.filter((fieldReq) => {
      return !template.requiredBindings.some((binding) =>
        binding.nodeIds.includes(fieldReq.nodeId),
      );
    });
  }, [template]);
  const getEffectiveFieldValue = useCallback(
    (fieldReq: NonNullable<WorkflowTemplate["fieldRequirements"]>[number]) => {
      ...
      for (const [index, binding] of template.requiredBindings.entries()) {
        ...
        if (
          ...
          platformCredentials.some(
            (credential) => credential.id === previousSelection,
          )
        ) {
  ...
  const missingPlatforms = useMemo(() => {
    ...
    const availablePlatforms = new Set(
      credentials.map((c) => c.platform.toLowerCase()),
    );
  ...
  const canProceed = useMemo(() => {
    ...
    const allCredentialsSelected = template.requiredBindings.every(
      (binding, index) =>
        selectedBindings[getTemplateBindingKey(binding, index)],
    );
    const allRequiredFieldsFilled =
      !template.fieldRequirements ||
      template.fieldRequirements.every((fieldReq) => {
        if (!fieldReq.required) return true;
        ...
        const isNotionIdField =
          fieldReq.field === "databaseId" ||
  ...
  const renderFieldControl = useCallback(
    (fieldReq: NonNullable<WorkflowTemplate["fieldRequirements"]>[number]) => {
      ...
      if (fieldReq.type === "select") {
        return (
          <select
            className="w-full px-3 py-2 rounded-md border border-white/10 bg-white/5 text-sm text-white"
            value={currentValue}
            onChange={(e) =>
              handleFieldChange(fieldReq.nodeId, fieldReq.field, e.target.value)
            }
          >
            {!fieldReq.defaultValue && (
              <option value="">-- Select {fieldReq.label} --</option>
            )}
            {fieldReq.options?.map((opt) => (
      ...
      const normalizedNotionId = isNotionIdField
        ...
        : undefined;
      const notionFieldIsTemplate =
        isNotionIdField && looksLikeTemplateExpression(currentValue);
      const isInvalidNotionId =
        isNotionIdField &&
      ...
      return (
        <>
          <input
            ...
            className="w-full px-3 py-2 rounded-md border border-white/10 bg-white/5 text-sm text-white placeholder:text-white/30"
            value={currentValue}
            onChange={(e) =>
              handleFieldChange(fieldReq.nodeId, fieldReq.field, e.target.value)
            }
          ...
          {isInvalidNotionId && (
            <div className="text-xs text-amber-300 mt-1">
              Enter a valid Notion URL or ID. Placeholder/all-zero IDs are not
              allowed.
            </div>
          )}
          {isNotionIdField && (
          ...
          {fieldReq.placeholder.includes("{") && (
            <div className="text-xs text-white/40 mt-1">
              Tip: You can use template variables like{" "}
              {fieldReq.placeholder.match(/\{[^}]+\}/)?.[0] ||
  ...
  const handleConfirm = async () => {
    if (!template || !canProceed) return;
    const bindings: CredentialBinding[] = template.requiredBindings.map(
      (binding, index) => {
        const bindingKey = getTemplateBindingKey(binding, index);
        const credentialId = selectedBindings[bindingKey];
        if (!credentialId) {
          throw new Error(
            `Missing credential binding for ${binding.platform} (${bindingKey})`,
          );
        }
        return {
          bindingKey,
    ...
    try {
      ...
      for (const [nodeId, fields] of Object.entries(additionalFields)) {
        ...
        for (const [field, rawValue] of Object.entries(fields)) {
          const isNotionIdField =
            ...
            field === "dataSourceId" ||
            field === "pageId";
          if (!isNotionIdField) continue;
          if (looksLikeTemplateExpression(rawValue)) continue;
      ...
      setSubmitError(
        error instanceof Error
          ? error.message
          : "Failed to apply template setup.",
      );
    } finally {
      setIsSubmitting(false);
    }
      ...
      <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-4">
          {template.requiredBindings.map((binding, index) => {
            const bindingKey = getTemplateBindingKey(binding, index);
            const platformCredentials = credentials.filter(
              (c) => c.platform.toLowerCase() === binding.platform,
            );
            ...
            const hasDuplicatePlatform =
              (platformBindingCounts.get(binding.platform) ?? 0) > 1;
            const bindingHint = hasDuplicatePlatform
              ? (binding.nodeIds[0] ?? `binding ${index + 1}`)
              : null;
            return (
              <div key={bindingKey} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {hasMissing && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleGoToCredentials(binding.platform)}
                    >
                ...
                {platformCredentials.length > 0 ? (
                  <select
                    ...
                    value={selected || ""}
                    onChange={(e) =>
                      handleCredentialSelect(bindingKey, e.target.value)
                    }
                  >
                    <option value="">
                      -- Select {binding.platform} credential --
                    </option>
                  ...
                  <div className="px-3 py-2 rounded-md border border-amber-500/30 bg-amber-500/10 text-xs text-amber-200 flex items-center justify-between">
                    <span>
                      No {binding.platform} credentials found. Create one in
                      Credentials.
                    </span>
                    <Button
                      size="sm"
                      variant="outline"
                      className="h-7 text-[11px] bg-black text-white border-black hover:bg-neutral-900 hover:text-white"
                      onClick={() => handleGoToCredentials(binding.platform)}
                    >
                      Go
                    </Button>
                ...
                {selected &&
                  template.fieldRequirements
                    ...
                    .map((fieldReq) => (
                      <div
                        key={`${fieldReq.nodeId}-${fieldReq.field}`}
                        className="space-y-2 mt-3 pl-3 border-l-2 border-white/10"
                      >
                        <div>
          ...
          {unboundFieldRequirements.length > 0 && (
            <div className="space-y-2">
              ...
              {unboundFieldRequirements.map((fieldReq) => (
                <div
                  key={`unbound-${fieldReq.nodeId}-${fieldReq.field}`}
                  className="space-y-2 pl-3 border-l-2 border-white/10"
                >
                  <div>
                    <label className="text-xs text-white/70 mb-1 block">
                      {fieldReq.label}{" "}
          ...
          {template.setupMode === "needs_input" &&
            template.inputRequirements &&
            template.inputRequirements.length > 0 && (
              <div className="mt-4 p-3 rounded-md border border-blue-500/30 bg-blue-500/10">
                <div className="text-sm font-medium text-blue-200 mb-2">
                  Additional Setup Required
                </div>
                <div className="text-xs text-blue-200/80 space-y-1">
                  {template.inputRequirements.map((req, idx) => (
                    <div key={idx}>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            className="bg-white hover:bg-gray-100 text-black disabled:bg-white/50 disabled:text-black/50"
          >
