...
function isPlaceholderSecret(value: string): boolean {
  ...
  return (
    normalized === WEBHOOK_SECRET_PLACEHOLDER ||
...
export function WebhookTriggerConfig({
  ...
  const [secret, setSecret] = useState(
    isPlaceholderSecret(initialSecret) ? "" : initialSecret,
  );
  ...
  const webhookUrl = useMemo(() => {
    ...
    const baseUrl =
      publicAppUrl && /^https?:\/\//.test(publicAppUrl)
  ...
  if (isLabelMissing) missingFields.push("Label");
  if (isSecretMissing) missingFields.push("Secret");
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        <Label className="text-white/80">
          ...
          {isLabelMissing && (
            <span
              className="ml-1.5 inline-flex h-4 w-4 items-center justify-center rounded-full bg-red-500/15 text-[10px] font-bold leading-none text-red-400"
        ...
        <Input
          ...
          className={`bg-[#2D2D2E] text-white placeholder:text-white/30 ${isLabelMissing ? "border-red-500/70 focus-visible:ring-red-500/40" : "border-[#444]"}`}
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          aria-invalid={isSecretMissing}
          className={`bg-[#2D2D2E] text-white placeholder:text-white/30 ${isSecretMissing ? "border-red-500/70 focus-visible:ring-red-500/40" : "border-[#444]"}`}
        />
        {isSecretMissing && (
          <p className="text-xs text-red-400">Secret is required.</p>
        )}
        <p className="text-xs text-white/40">
          Requests must send this as query `secret` or header `x-fynt-secret`.
        </p>
      </div>
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-white/40">
          Helpful when using condition routing from payload fields.
        </p>
      </div>
      ...
      <div className="space-y-2">
        ...
        {typeof window !== "undefined" &&
          ...
          !publicAppUrl && (
            <p className="text-xs text-white/40">
              This localhost URL works only on your machine. Set{" "}
              <code>NEXT_PUBLIC_APP_URL</code> for a public URL.
            </p>
          )}
      </div>
      ...
      <Button
        type="submit"
        disabled={isLabelMissing || isSecretMissing}
