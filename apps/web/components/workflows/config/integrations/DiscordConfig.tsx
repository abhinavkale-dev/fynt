...
export function DiscordConfig({ data, onSave }: DiscordConfigProps) {
  const [responseName, setResponseName] = useState(
    data.responseName || "discordResponse",
  );
  ...
  const discordCredentials = (credentialsQuery.data ?? []).filter(
    (credential) => credential.platform === "discord",
  );
  ...
  const isSaveDisabled =
    !responseName.trim() || !credentialId || !content.trim();
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          placeholder="Summary: The user has 100 points."
          rows={4}
        ...
        />
        <p className="text-xs text-white/40">
          The content to send to the Discord webhook. Use {`{{variables}}`} for
