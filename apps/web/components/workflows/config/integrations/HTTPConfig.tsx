...
export function HTTPConfig({ data, onSave }: HTTPConfigProps) {
  const [responseName, setResponseName] = useState(
    data.responseName || "httpResponse",
  );
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        ...
        <p className="text-xs text-white/40">
          Use this name to reference the response in the other nodes:{" "}
          {`{{${responseName}.data}}`}
        </p>
      </div>
          ...
          {headers.map((header, index) => (
            <div key={index} className="flex gap-2 items-center">
              <Input
                ...
                placeholder="Key"
                className="bg-[#2D2D2E] border-[#444] text-white placeholder:text-white/30 flex-1"
              />
              <Input
                value={header.value}
      ...
      {showBody && (
        <div className="space-y-2">
          ...
          <textarea
            ...
            rows={4}
            className="w-full rounded-md border border-[#444] bg-[#2D2D2E] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-1 focus:ring-ring resize-none font-mono"
          />
          <p className="text-xs text-white/40">
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          placeholder="30000"
          className="bg-[#2D2D2E] border-[#444] text-white placeholder:text-white/30 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
        />
        <p className="text-xs text-white/40">
