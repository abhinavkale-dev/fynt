...
function normalizeNotionIdentifier(
...
export function NotionConfig({ data, onSave }: NotionConfigProps) {
  ...
  const [childrenTemplate, setChildrenTemplate] = useState(
    data.childrenTemplate || "",
  );
  ...
  const [pageSize, setPageSize] = useState(
    typeof data.pageSize === "number" ? String(data.pageSize) : "25",
  );
  const [startCursor, setStartCursor] = useState(data.startCursor || "");
  const [archived, setArchived] = useState<ArchivedState>(
    typeof data.archived === "boolean"
      ? (String(data.archived) as ArchivedState)
      : "",
  );
  ...
  const hasValidTargetId =
    ...
    (Boolean(normalizedTargetId) &&
      !isClearlyInvalidNotionId(normalizedTargetId!));
  ...
  const notionCredentials = useMemo(
    () =>
      (credentialsQuery.data ?? []).filter(
        (credential) => credential.platform === "notion",
      ),
    [credentialsQuery.data],
  );
  const missingFields: string[] = [];
  if (!label.trim()) missingFields.push("Label");
  if (!responseName.trim()) missingFields.push("Response Name");
  if (!credentialId) missingFields.push("Credential");
  if (operation === "create") {
    if (!targetId.trim()) missingFields.push("Data Source ID or Database ID");
    else if (!hasValidTargetId)
      missingFields.push("Valid Data Source ID or Database ID");
    if (!propertiesTemplate.trim()) missingFields.push("Properties Template");
  }
  if (operation === "get") {
    if (!pageId.trim()) missingFields.push("Page ID");
    else if (!hasValidPageId) missingFields.push("Valid Page ID");
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      credentialId,
      operation,
      dataSourceId: targetIdIsTemplate
        ? targetId.trim()
        : (normalizedTargetId ?? targetId.trim()),
      databaseId: targetIdIsTemplate
        ? targetId.trim()
        : (normalizedTargetId ?? targetId.trim()),
      pageId: pageIdIsTemplate
        ? pageId.trim()
        : (normalizedPageId ?? pageId.trim()),
      propertiesTemplate,
      childrenTemplate,
      filterTemplate,
      sortsTemplate,
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        ...
        <Input
          ...
          placeholder="Notion"
          className="bg-[#2D2D2E] border-[#444] text-white"
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          placeholder="notionResponse"
          className="bg-[#2D2D2E] border-[#444] text-white placeholder:text-white/30"
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <select
          ...
          className="w-full h-9 rounded-md border border-[#444] bg-[#2D2D2E] px-3 text-sm text-white focus:outline-none"
        >
        ...
        <p className="text-xs text-white/40">
          ...
          <a
            ...
            className="text-[#F04D26] hover:text-[#F04D26]/90 underline"
          >
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-white/40">
          Quick setup: just paste a Notion link. Use a database link for
          Create/Get Many, and a page link for Get/Update.
        </p>
      </div>

      {(operation === "create" || operation === "get_many") && (
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-white/40">
            Copy link from the database in Notion and paste it here. We
            auto-extract the ID. Raw ID also works.
          </p>
          <p className="text-xs text-white/40">
            For this operation, use a database link (not a regular page link).
          </p>
          {targetId.trim() && !hasValidTargetId && (
            <p className="text-xs text-amber-300">
              Could not parse a Notion database ID from this value. Paste a
              database link or valid ID.
            </p>
          )}
        </div>
      )}
      ...
      {(operation === "get" || operation === "update") && (
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-white/40">
            Copy link from the page in Notion and paste it here. We auto-extract
            the page ID. Raw ID also works.
          </p>
          {pageId.trim() && !hasValidPageId && (
            <p className="text-xs text-amber-300">
      ...
      {(operation === "create" || operation === "update") && (
        <div className="space-y-2">
          ...
          <textarea
            ...
            onChange={(e) => setPropertiesTemplate(e.target.value)}
            rows={5}
            className="w-full rounded-md border border-[#444] bg-[#2D2D2E] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none resize-none font-mono"
            placeholder='{"Name":{"title":[{"text":{"content":"Hello"}}]}}'
          />
          <p className="text-xs text-white/40">
            Must resolve to a JSON object. Use {`{json variable.path}`} for
            objects/arrays.
          </p>
        </div>
      )}
      ...
      {operation === "create" && (
        <div className="space-y-2">
          <Label className="text-white/80" optional>
            Page Content Template (JSON Array)
          </Label>
          <textarea
            value={childrenTemplate}
            onChange={(e) => setChildrenTemplate(e.target.value)}
            rows={5}
            className="w-full rounded-md border border-[#444] bg-[#2D2D2E] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none resize-none font-mono"
            placeholder='[{"object":"block","type":"paragraph","paragraph":{"rich_text":[{"type":"text","text":{"content":"Entry details"}}]}}]'
          />
          <p className="text-xs text-white/40">
            Optional Notion page blocks. Must resolve to a JSON array. Useful
            for AI summaries and report details.
          </p>
        </div>
      )}
        ...
        <>
          <div className="space-y-2">
            ...
            <textarea
              ...
              rows={4}
              className="w-full rounded-md border border-[#444] bg-[#2D2D2E] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none resize-none font-mono"
              placeholder='{"property":"Status","select":{"equals":"Todo"}}'
            />
          </div>

          <div className="space-y-2">
            <Label className="text-white/80" optional>
              Sorts Template (JSON Array)
            </Label>
            <textarea
              value={sortsTemplate}
              onChange={(e) => setSortsTemplate(e.target.value)}
              rows={3}
              className="w-full rounded-md border border-[#444] bg-[#2D2D2E] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none resize-none font-mono"
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="next_cursor from prior response"
              className="bg-[#2D2D2E] border-[#444] text-white placeholder:text-white/30"
            />
          </div>
        </>
      ...
      {operation === "update" && (
        <div className="space-y-2">
          ...
          <select
            value={archived}
            onChange={(e) => setArchived(e.target.value as ArchivedState)}
            className="w-full h-9 rounded-md border border-[#444] bg-[#2D2D2E] px-3 text-sm text-white focus:outline-none"
          >
