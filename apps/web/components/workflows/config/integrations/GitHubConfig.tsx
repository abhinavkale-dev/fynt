...
export function GitHubConfig({ data, onSave }: GitHubConfigProps) {
  ...
  const [responseName, setResponseName] = useState(
    data.responseName || "githubResponse",
  );
  const [credentialId, setCredentialId] = useState(data.credentialId || "");
  const [operation, setOperation] = useState<GitHubOperation>(
    data.operation === "create_comment" || data.operation === "get_issue"
      ? data.operation
      : "create_issue",
  );
  ...
  const [labelsTemplate, setLabelsTemplate] = useState(
    data.labelsTemplate || "",
  );
  ...
  const githubCredentials = useMemo(
    () =>
      (credentialsQuery.data ?? []).filter(
        (credential) => credential.platform === "github",
      ),
  ...
  if (!repo.trim()) missingFields.push("Repository");
  if (operation === "create_issue") {
    if (!title.trim()) missingFields.push("Issue Title");
  }
  if (operation === "create_comment" || operation === "get_issue") {
    if (!issueNumber.trim()) missingFields.push("Issue Number");
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <select
          value={operation}
          onChange={(e) => setOperation(e.target.value as GitHubOperation)}
          className="w-full h-9 rounded-md border border-[#444] bg-[#2D2D2E] px-3 text-sm text-white focus:outline-none"
        >
      ...
      {(operation === "create_comment" || operation === "get_issue") && (
        <div className="space-y-2">
        ...
        <>
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Bug: API timeout in production"
              className="bg-[#2D2D2E] border-[#444] text-white placeholder:text-white/30"
            />
          </div>
          <div className="space-y-2">
            ...
            <textarea
              ...
              rows={4}
              className="w-full rounded-md border border-[#444] bg-[#2D2D2E] px-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none resize-none"
              placeholder="Issue details..."
            />
          </div>
          <div className="space-y-2">
            <Label className="text-white/80" optional>
              Labels Template (JSON Array)
            </Label>
            <Input
              value={labelsTemplate}
              onChange={(e) => setLabelsTemplate(e.target.value)}
              placeholder='["bug","high-priority"]'
              className="bg-[#2D2D2E] border-[#444] text-white placeholder:text-white/30 font-mono"
            />
          </div>
        </>
      )}
