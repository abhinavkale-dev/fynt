...
export function OpenAIConfig({ data, onSave }: OpenAIConfigProps) {
  const [responseName, setResponseName] = useState(
    data.responseName || "openaiResponse",
  );
  ...
  const openAICredentials = (credentialsQuery.data ?? []).filter(
    (credential) => credential.platform === "openai",
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-white/40">
          Prompt mode sends a single user prompt. JSON mode expects full OpenAI
          chat input with messages[].
        </p>
      </div>
      ...
      {inputMode === "prompt" ? (
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            placeholder={OPENAI_JSON_PLACEHOLDER}
            rows={8}
          ...
          />
          <p className="text-xs text-white/40">
            OpenAI JSON mode requires a non-empty messages[] array with valid
            roles and content.
          </p>
        </div>
      )}
