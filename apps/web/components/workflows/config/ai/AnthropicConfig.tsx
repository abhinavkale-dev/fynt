...
export function AnthropicConfig({ data, onSave }: AnthropicConfigProps) {
  const [responseName, setResponseName] = useState(
    data.responseName || "anthropicResponse",
  );
  const [credentialId, setCredentialId] = useState(data.credentialId || "");
  const [model, setModel] = useState(data.model || "");
  const [systemPrompt, setSystemPrompt] = useState(data.systemPrompt || "");
  const [inputMode, setInputMode] = useState<"prompt" | "json">(
    data.inputMode === "json" ? "json" : "prompt",
  );
  ...
  const anthropicCredentials = (credentialsQuery.data ?? []).filter(
    (credential) => credential.platform === "anthropic",
  );
  ...
  const inputMissing =
    inputMode === "json" ? !requestJson.trim() : !prompt.trim();
  const isSaveDisabled =
    !responseName.trim() || !credentialId || !model.trim() || inputMissing;
  ...
  if (inputMode === "json") {
    if (!requestJson.trim()) missingFields.push("JSON Input");
  } else if (!prompt.trim()) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <select
          ...
          className="w-full h-9 rounded-md border border-[#444] bg-[#2D2D2E] px-3 text-sm text-white focus:outline-none focus:ring-1 focus:ring-ring"
        >
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-white/40">
          Sets the behavior of the AI model. Use {`{{variables}}`} for simple
      ...
      <div className="space-y-2">
        ...
        <select
          value={inputMode}
          onChange={(e) => setInputMode(e.target.value as "prompt" | "json")}
          className="w-full h-9 rounded-md border border-[#444] bg-[#2D2D2E] px-3 text-sm text-white focus:outline-none focus:ring-1 focus:ring-ring"
        >
        ...
        <p className="text-xs text-white/40">
          Prompt mode sends one user prompt. JSON mode expects Anthropic
          messages[] (roles user/assistant).
        </p>
      </div>
      ...
      {inputMode === "prompt" ? (
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-white/40">
            The prompt to send to the AI model. Use {`{{variables}}`} for simple
            values or {`{{json variables}}`} to stringify objects.
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          ...
          <textarea
            ...
            placeholder={ANTHROPIC_JSON_PLACEHOLDER}
            rows={8}
            className="w-full rounded-md border border-[#444] bg-[#2D2D2E] px-3 py-2 font-mono text-xs text-white placeholder:text-white/30 focus:outline-none focus:ring-1 focus:ring-ring resize-y"
          />
          <p className="text-xs text-white/40">
            Anthropic JSON mode requires a non-empty messages[] array using role
            user/assistant.
          </p>
        </div>
      )}
