...
export function GeminiConfig({ data, onSave }: GeminiConfigProps) {
  const [responseName, setResponseName] = useState(
    data.responseName || "geminiResponse",
  );
  ...
  const geminiCredentials = (credentialsQuery.data ?? []).filter(
    (credential) => credential.platform === "gemini",
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-white/40">
          Prompt mode sends one prompt. JSON mode expects Gemini contents[] with
          parts[] text blocks.
        </p>
      </div>
      ...
      {inputMode === "prompt" ? (
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            placeholder={GEMINI_JSON_PLACEHOLDER}
            rows={8}
          ...
          />
          <p className="text-xs text-white/40">
            Gemini JSON mode requires contents[] entries with parts[] and at
            least one text part.
          </p>
        </div>
      )}
