  ...
  return (
    <Panel position="bottom-left" className="m-4">
      <div className="flex flex-col gap-1">
        <button
          onClick={() => zoomIn()}
          className="w-8 h-8 bg-[#1E1E1E] hover:bg-[#2A2A2A] border border-[#333333] hover:border-[#F04D26] rounded flex items-center justify-center transition-[background-color,border-color] duration-150 ease group"
          title="Zoom In"
        >
          <svg
        ...
        <button
          onClick={onTidyUp}
          disabled={!hasNodes}
          className="w-8 h-8 bg-[#1E1E1E] hover:bg-[#2A2A2A] border border-[#333333] hover:border-[#F04D26] rounded flex items-center justify-center transition-[background-color,border-color,opacity] duration-150 ease group disabled:opacity-50 disabled:cursor-not-allowed"
          title="Tidy workflow"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
...
const WorkflowCanvas: React.FC<WorkflowCanvasProps> = ({
  ...
  const hasActiveManualTrigger = useMemo(
    () =>
      nodes.some((node) => {
  ...
  const runNowCandidate = useMemo<{
    ...
    const activeCron = nodes.find(
      (node) => node.type === "cronTrigger" && isRunnableTriggerNode(node),
    );
    if (activeCron) {
      return { source: "cron", nodeId: activeCron.id };
    }
    const activeWebhook = nodes.find(
      (node) => node.type === "webhookTrigger" && isRunnableTriggerNode(node),
    );
    if (activeWebhook) {
  ...
  const isRunNowButtonDisabled =
    isExecuting || hasValidationErrors || isCronRunButton;
  const hasValidationWarnings = validation.issues.some(
    (i) => i.type === "warning",
  );
      ...
      <ContextMenuTrigger asChild>
        <div className="w-full h-full rounded-b-lg overflow-hidden">
          <ReactFlow
            ...
            minZoom={0.2}
            maxZoom={2}
            panOnScroll={hasNodes && isInteractive}
            panOnScrollSpeed={0.5}
            panOnDrag={hasNodes && isInteractive}
            zoomOnScroll={hasNodes}
            zoomOnPinch={hasNodes}
            zoomOnDoubleClick={hasNodes}
            nodesDraggable={hasNodes && isInteractive}
            nodesConnectable={hasNodes && isInteractive}
            elementsSelectable={hasNodes && isInteractive}
            colorMode="dark"
                ...
                <TooltipProvider>
                  <Tooltip>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          disabled={
                            isExecuting ||
                            hasValidationErrors ||
                            disableManualExecute
                          }
                          className={`flex items-center gap-2 px-4 py-2 text-white text-sm font-medium rounded-lg transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed ${
                            hasValidationErrors || disableManualExecute
                        ...
                        {canRunNowViaTrigger && runNowCandidate && (
                          <button
                            onClick={handleRunNowViaTrigger}
                            disabled={isRunNowButtonDisabled}
                            className={`flex items-center gap-2 px-4 py-2 text-white text-sm font-medium rounded-lg transition-all shadow-lg disabled:cursor-not-allowed ${
                              hasValidationErrors
                    ...
                    <TooltipContent side="top" className="max-w-xs">
                      {disableManualExecute && !hasValidationErrors && (
                      ...
                      {isCronRunButton && !hasValidationErrors && (
                        <div className="text-xs space-y-1">
                          <div className="font-semibold text-emerald-300">
                            Cron is active
                          </div>
                          <div className="text-white/90">
                            This workflow runs automatically by schedule.
                          </div>
                        </div>
                      )}
                      {hasValidationErrors && (
                      ...
                      {hasValidationWarnings && !hasValidationErrors && (
                        <div className="text-xs space-y-1">
                          ...
                          <div className="mt-2 text-white/70">
                            You can still execute, but results may be unexpected
                          </div>
                        </div>
                      )}
