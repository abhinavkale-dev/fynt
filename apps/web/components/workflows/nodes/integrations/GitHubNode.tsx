...
function getSubtitle(data: GitHubNodeData, isConfigured: boolean): string {
  if (isConfigured) {
    ...
    if (data.operation === "create_comment") return "Create issue comment";
    if (data.operation === "get_issue") return "Get issue";
    return "Create issue";
  }
  if (!data.credentialId) return "Add credential";
  return "Configure repository fields";
}
export default function GitHubNode({
  data,
  id,
  selected,
}: {
