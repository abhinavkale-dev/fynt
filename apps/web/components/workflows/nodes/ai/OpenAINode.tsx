...
function getSubtitle(data: OpenAINodeData, isConfigured: boolean): string {
  ...
  if (isConfigured) {
    return data.model
      ? `Model: ${data.model} (${inputMode})`
      : data.subtitle || "Generate response";
  }
  const missing: string[] = [];
  if (!data.credentialId?.trim()) missing.push("credential");
  if (!data.model?.trim()) missing.push("model");
  if (inputMode === "json") {
    if (!data.requestJson?.trim()) missing.push("json input");
...
export default function OpenAINode({
  ...
  const inputMode = resolveInputMode(data);
  const hasInput =
    inputMode === "json" ? !!data.requestJson?.trim() : !!data.prompt?.trim();
  const isConfigured = !!(
    ...
    <BaseNode
      ...
      icon={
        <Image
          ...
          height={40}
          src="/logo/openai.svg"
          className="size-10 object-contain rounded-sm grayscale opacity-80"
        />
      }
      label={data.label || "OpenAI"}
