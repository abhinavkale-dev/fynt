...
export default function BaseNode({
  ...
  const isManualTriggerNode =
    isTrigger && (nodeType === "triggerNode" || nodeType === "manualTrigger");
  return (
    <motion.div
      className="relative group flex flex-col items-center"
      initial={{ opacity: 0, scale: 0.95 }}
      animate={{ opacity: 1, scale: 1 }}
      transition={NODE_ENTRANCE.spring}
    >
      <div className="relative">
        {isManualTriggerNode && canEditNodes && (
          <div className="absolute top-1/2 -left-4 -translate-x-full -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-150 ease-out nodrag z-10 flex items-center gap-1">
            <button
              onClick={handleExecuteWorkflow}
              className="flex items-center gap-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 disabled:bg-[#2D2D2E] disabled:text-white/50 disabled:cursor-not-allowed text-white text-sm font-medium rounded-lg transition-colors shadow-lg whitespace-nowrap"
              title={
                isNodeActive ? "Execute workflow" : "Trigger is deactivated"
              }
            ...
            >
              ...
              >
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M13.4868 7.0974C13.4955 6.98275 13.5 6.8669 13.5 6.75C13.5 4.26472 11.4853 2.25 9 2.25C6.51472 2.25 4.5 4.26472 4.5 6.75C4.5 6.86689 4.50446 6.98275 4.51321 7.0974C2.89021 7.777 1.75 9.38035 1.75 11.25C1.75 13.7353 3.76472 15.75 6.25 15.75C7.28562 15.75 8.23953 15.4002 9 14.8122C9.76047 15.4002 10.7144 15.75 11.75 15.75C14.2353 15.75 16.25 13.7353 16.25 11.25C16.25 9.38035 15.1098 7.77701 13.4868 7.0974Z"
                  fill="rgba(255, 255, 255, 1)"
                  fillOpacity="0.3"
                  data-color="color-2"
                ></path>
                <path
                  d="M10.496 9.757C10.66 10.224 10.75 10.727 10.75 11.25C10.75 13.735 8.735 15.75 6.25 15.75C3.765 15.75 1.75 13.735 1.75 11.25C1.75 10.339 2.021 9.491 2.486 8.783"
                ...
                ></path>
                <path
                  d="M11.511 15.745C12.042 15.773 12.587 15.707 13.123 15.536C15.49 14.778 16.794 12.245 16.036 9.878C15.278 7.511 12.745 6.207 10.378 6.965C9.50999 7.243 8.78599 7.759 8.25299 8.418"
                  stroke="rgba(255, 255, 255, 1)"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                ...
                ></path>
                <path
                  d="M8.156 11.171C7.695 11.083 7.239 10.92 6.806 10.679C4.636 9.468 3.859 6.727 5.07 4.556C6.281 2.385 9.022 1.609 11.193 2.82C11.904 3.217 12.465 3.778 12.856 4.429"
                  stroke="rgba(255, 255, 255, 1)"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
            ...
            <button
              ...
              className={`w-8 h-8 border rounded flex items-center justify-center transition-[color,background-color,transform] duration-100 ease-out active:scale-[0.95] ${
                isNodeActive
                  ? "bg-[#2D2D2E] hover:bg-[#3e3e3e] border-[#5b5b5b] text-white"
                  : "bg-[#3A2A2A] hover:bg-[#4a3030] border-[#6f4f4f] text-[#ffb6b6]"
              }`}
              title={isNodeActive ? "Deactivate node" : "Activate node"}
              aria-label={isNodeActive ? "Deactivate node" : "Activate node"}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                x="0px"
        ...
        {!isManualTriggerNode && canEditNodes && (
          <div className="absolute -top-10 left-1/2 -translate-x-1/2 flex gap-1 opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-[opacity,transform] duration-150 ease-out nodrag z-10">
            <button
              className="w-8 h-8 bg-[#2D2D2E] hover:bg-[#3e3e3e] disabled:bg-[#2D2D2E] disabled:text-white/50 disabled:cursor-not-allowed border border-[#5b5b5b] rounded flex items-center justify-center text-white transition-[color,background-color,transform] duration-100 ease-out active:scale-[0.95]"
              onClick={handleExecuteStep}
              title={isNodeActive ? "Execute step" : "Node is deactivated"}
            ...
            <button
              className="w-8 h-8 bg-[#2D2D2E] hover:bg-[#3e3e3e] border border-[#5b5b5b] rounded flex items-center justify-center text-white transition-[color,background-color,transform] duration-100 ease-out active:scale-[0.95]"
                ...
                : executionStatus === "Success"
                  ...
                  : executionStatus === "Skipped"
                    ...
                    : executionStatus === "Failed"
                      ...
                      : !isNodeActive
                        ? "border-[#4a4a4a] shadow-none"
                        : selected
                          ? "border-[#999] shadow-lg"
                          : "border-[#5b5b5b]"
            }
        ...
        >
          {!isNodeActive && (
            <div className="absolute top-2 right-2 z-10 rounded bg-[#2D2D2E] px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-white/65 border border-[#5b5b5b]">
              Off
            </div>
          )}
          ...
          {executionStatus && (
            <div className="absolute bottom-2 right-2 z-10">
              {(executionStatus === "Running" ||
                ...
                <div className="relative w-4 h-4 rounded-full bg-[#2D2D2E] border border-yellow-500 flex items-center justify-center overflow-hidden">
                  <svg
                    className="absolute inset-0 size-4 text-yellow-400 animate-[spin_1.6s_linear_infinite]"
                    viewBox="0 0 16 16"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
              ...
              {executionStatus === "Success" && (
                <div className="w-4 h-4 rounded-full bg-[#2D2D2E] border border-green-500 flex items-center justify-center">
                  <div className="w-2 h-2 rounded-full bg-green-500" />
                </div>
              )}
              {executionStatus === "Skipped" && (
                <div className="w-4 h-4 rounded-full bg-[#2D2D2E] border border-[#6b7280] flex items-center justify-center">
                  <div className="w-2 h-2 rounded-full bg-[#9ca3af]" />
                </div>
              )}
              {(executionStatus === "Failed" ||
                executionStatus === "Failure") && (
                <div className="w-4 h-4 rounded-full bg-[#2D2D2E] border border-red-500 flex items-center justify-center">
          ...
          {!isConfigured && !executionStatus && isNodeActive && (
              ...
              <div className="relative">
                ...
                >
                  <path
                    fill="currentColor"
                    fillRule="evenodd"
                    d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0zM11 9a1 1 0 0 1 2 0v4a1 1 0 1 1-2 0V9zm1 8a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"
                    clipRule="evenodd"
                  />
                </svg>
              </div>
          ...
          {!isTrigger && (
            <Handle
              ...
              className="w-4! h-4! bg-[#5b5b5b]! border-2! border-[#2D2D2E]! rounded-full transition-[background-color,transform] duration-150 ease-out hover:bg-[#999]! hover:scale-110!"
            />
          )}
          ...
          {sourceHandles?.map((handle) => (
            <div key={handle.id}>
              <Handle
                ...
                className="w-3! h-3! bg-[#6f6f6f]! border-2! border-[#2D2D2E]! rounded-full transition-[background-color,transform] duration-150 ease-out hover:bg-[#ddd]! hover:scale-110!"
              />
              {handle.label && (
                <span
                  className="absolute text-[10px] text-white/50 uppercase tracking-wide"
                  style={{
                    top: handle.top ?? "50%",
                    right: "-56px",
      ...
      <div className="mt-3 flex flex-col items-center text-center pointer-events-none">
        ...
        <p className="text-[#999] text-xs whitespace-nowrap">{subtitle}</p>
      </div>
      ...
      {!openNodeConfig && (
        <NodeConfigDialog
          ...
          onSave={(nodeId, data) => updateNodeData(nodeId, data)}
        />
      )}
    </motion.div>
  );
