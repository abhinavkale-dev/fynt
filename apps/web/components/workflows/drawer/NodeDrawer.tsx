        ...
        {CATEGORY_ORDER.map((category, catIdx) => {
          ...
          return (
            ...
            >
              <CollapsibleTrigger
                asChild
                aria-label={`Toggle ${CATEGORY_LABELS[category]} category`}
              >
                <motion.button
                  ...
                  className="flex items-center gap-2 w-full text-left py-1.5 px-2 -mx-2 rounded-md hover:bg-white/5 transition-colors duration-150"
                >
                  <ChevronRight
                    className={`size-3.5 text-white/40 shrink-0 transition-transform duration-200 ${openCategories[category] ? "rotate-90" : ""}`}
                  />
              ...
              <CollapsibleContent className="overflow-hidden transition-all duration-200 ease-out">
                ...
                >
                  {defs.map((def) => {
                    ...
                    const isDisabled =
                      def.isTrigger && hasTrigger && def.maxPerWorkflow === 1;
                    return (
                      <motion.button
                        key={def.type}
                        ...
                        aria-label={`Add ${def.label} ${def.isTrigger ? "trigger" : "action"}`}
                      >
                        <div
                          className={`shrink-0 w-8 h-8 rounded-md border flex items-center justify-center text-muted-foreground ${tint}`}
                        >
                          {ui?.icon}
                        </div>
