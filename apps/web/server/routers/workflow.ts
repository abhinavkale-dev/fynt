...
import {
  RunReservationLockTimeoutError,
  withUserRunReservationLock,
} from "@repo/shared/run-reservation-lock";
...
function normalizeWorkflowGraph<
  T extends {
    nodes: unknown;
    edges: unknown;
  },
...
function assertGraphShapeOrThrow(
  nodes: unknown[],
  edges: unknown[],
...
function assertTemplateGraphNotEmptyOrThrow(
  nodes: unknown[],
  ...
  if (nodes.length === 0) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message:
        `Invalid workflow graph for ${context}: template workflow "${templateId}" ` +
...
function resolveExecutionStreamSigningSecret(): string {
  ...
  if (!secret) {
    throw new Error(
      "Execution stream signing secret is required. Set BETTER_AUTH_SECRET or EXECUTION_STREAM_SIGNING_SECRET.",
    );
  }
  return secret;
}
...
function resolveExecutionStreamWsBaseUrlFromEnv(): string | undefined {
  ...
  try {
    ...
    console.warn(
      "[workflow.execute] Invalid EXECUTION_WS_URL/NEXT_PUBLIC_EXECUTION_WS_URL. Falling back to client defaults.",
      error,
    );
    return undefined;
  }
...
const getAllSummaries = protectedProcedure.query(async ({ ctx }) => {
  ...
  return workflows.map((workflow) => ({
    ...
    lastRunStatus: workflow.lastRunStatus as
      | "Pending"
      | "Success"
      | "Failure"
      | null,
...
const create = protectedProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    assertTemplateGraphNotEmptyOrThrow(
      input.nodes,
      extractTemplateIdFromMetadata(input.triggerMetadata),
      "create",
    );
    ...
    const now = new Date();
    const effectivePlan =
      user.plan !== "free" && user.paidUntil && user.paidUntil < now
        ? "free"
        : user.plan;
    if (effectivePlan === "free" && workflowCount >= 30) {
      throw new TRPCError({
        code: "FORBIDDEN",
        message:
          "Free plan allows up to 30 workflows. Upgrade your plan for unlimited workflows.",
      });
    }
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    if (isPositionOnlySave) {
      if (input.nodes !== undefined || input.edges !== undefined) {
        assertGraphShapeOrThrow(input.nodes ?? [], input.edges ?? [], "update");
      }
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    if (input.nodeId && (input.triggerSource || input.triggerNodeId)) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message:
          "Provide either nodeId (single-node run) or triggerSource/triggerNodeId (trigger run), not both.",
      });
    }
    if (
      (input.triggerSource && !input.triggerNodeId) ||
      (!input.triggerSource && input.triggerNodeId)
    ) {
      throw new TRPCError({
        code: "BAD_REQUEST",
    ...
    if (input.nodeId) {
      const nodeExists = workflowNodes.some(
      ...
      const triggerNode = workflowNodes.find(
        (node) => node?.id === input.triggerNodeId,
      );
      if (!triggerNode) {
        throw new TRPCError({
          code: "BAD_REQUEST",
      ...
      const expectedType =
        input.triggerSource === "cron" ? "cronTrigger" : "webhookTrigger";
      const actualType =
        typeof triggerNode.type === "string" ? triggerNode.type : "";
      ...
      if (!hasActiveManualTrigger) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message:
            "Manual execution requires at least one active manual trigger node. Use webhook URL or cron schedule for this workflow.",
        });
      }
    }
    ...
    const effectiveLimit =
      effectivePlan === "free" ? 1000 : effectivePlan === "pro" ? 5000 : -1;
    const effectiveConcurrent =
      effectivePlan === "free" ? 2 : effectivePlan === "pro" ? 10 : 50;
    let workflowRun;
    try {
      workflowRun = await withUserRunReservationLock(ctx.userId!, () =>
      ...
      if (error instanceof RunReservationLockTimeoutError) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message:
            "Another execution request is being processed. Please retry in a moment.",
        });
      }
      throw error;
    }
    ...
    try {
      ...
      console.warn(
        "[workflow.execute] Failed to prepare execution stream bootstrap token.",
        error,
      );
    }
    return {
