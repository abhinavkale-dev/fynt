...
function maskKeys(keys: Record<string, unknown>): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(keys)) {
    ...
    masked[key] =
      str.length > 4 ? "•".repeat(str.length - 4) + str.slice(-4) : "••••";
  }
  return masked;
}
    ...
    try {
      ...
      if (
        ...
        throw new TRPCError({
          ...
          message:
            'Credential platform enum in database is out of sync. Run "pnpm prisma db push" from project root, then restart web/worker.',
        });
      }
      throw error;
    }
...
const update = protectedProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    if (input.keys) data.keys = encryptJson(input.keys);
