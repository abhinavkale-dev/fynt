...
function parseFormEncodedPayload(
  rawBody: string,
...
export function getWebhookRateLimitConfig() {
  return {
    ipPerMinute: parsePositiveInt(
      process.env.WEBHOOK_IP_RATE_LIMIT_PER_MINUTE,
      120,
    ),
    endpointPerMinute: parsePositiveInt(
      process.env.WEBHOOK_ENDPOINT_RATE_LIMIT_PER_MINUTE,
      60,
    ),
    bodyMaxBytes: parsePositiveInt(
      process.env.WEBHOOK_MAX_BODY_BYTES,
      256 * 1024,
    ),
  };
}
export async function readJsonOrTextWithLimit(
  request: NextRequest,
  maxBytes: number,
): Promise<{
  payload: unknown;
  ...
  if (contentLength) {
    ...
    if (Number.isFinite(declared) && declared > maxBytes) {
      throw new PayloadTooLargeError(
        `Payload too large. Limit is ${maxBytes} bytes.`,
      );
    }
  }
...
export function sanitizeRequestHeaders(
  ...
  request.headers.forEach((value, key) => {
    ...
    if (blockedHeaders.has(lowerKey)) return;
    if (count >= maxHeaderCount) return;
    const safeValue = truncateUtf8(value, maxHeaderValueBytes);
    const kvBytes =
      Buffer.byteLength(lowerKey, "utf8") +
      Buffer.byteLength(safeValue, "utf8");
    if (totalBytes + kvBytes > maxTotalBytes) return;
...
export function sanitizeQueryParams(
  ...
  request.nextUrl.searchParams.forEach((value, key) => {
    ...
    if (count >= maxParamCount) return;
    const safeValue = truncateUtf8(value, maxValueBytes);
    const kvBytes =
      Buffer.byteLength(key, "utf8") + Buffer.byteLength(safeValue, "utf8");
    if (totalBytes + kvBytes > maxTotalBytes) return;
