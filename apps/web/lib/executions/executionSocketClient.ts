...
function defaultRealtimeWsBaseUrl(): string {
  const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
  const realtimePort =
    process.env.NEXT_PUBLIC_REALTIME_PORT?.trim() || DEFAULT_REALTIME_PORT;
...
function resolveSocketBaseCandidates(
  wsUrlFromServer: string | undefined,
): string[] {
  const rawCandidates = [
...
function resolveSocketUrlPayloadFromToken(
  token: string,
  wsUrlFromServer: string | undefined,
  expiresInSeconds: number,
): ExecutionSocketUrlPayload {
    ...
    try {
      ...
      lastBuildError =
        error instanceof Error
          ? error
          : new Error("Failed to build websocket URL.");
    }
  }
  if (socketUrls.length === 0) {
    throw new Error(
      lastBuildError?.message || "Unable to resolve a valid websocket URL.",
    );
  }
  return {
    socketUrls,
...
function getPrimedSocketAuthPayload(
  runId: string,
): ExecutionSocketUrlPayload | null {
  ...
  try {
    return resolveSocketUrlPayloadFromToken(
      primed.token,
      primed.wsUrl,
      Math.max(1, Math.floor((primed.expiresAtMs - Date.now()) / 1000)),
    );
  } catch {
    PRIMED_SOCKET_AUTH.delete(runId);
    return null;
...
export function primeExecutionSocketAuth(
  runId: string,
  bootstrap: ExecutionSocketAuthBootstrap,
): void {
...
async function fetchSocketUrl(
  ...
  for (let attempt = 1; attempt <= WS_TOKEN_MAX_ATTEMPTS; attempt += 1) {
    ...
    failureDetail = await response
      .text()
      .catch(() => "Failed to fetch websocket token.");
    if (
      !shouldRetryTokenFetch(response.status) ||
      attempt >= WS_TOKEN_MAX_ATTEMPTS
    ) {
      throw new Error(
        failureDetail ||
          `Failed to fetch websocket token (${response.status}).`,
      );
    }
  ...
  private async ensureConnected(): Promise<void> {
    ...
    try {
      ...
      if (!socketUrl || Date.now() >= this.cachedSocketUrlExpiresAtMs) {
        const payload =
          getPrimedSocketAuthPayload(this.runId) ??
          (await fetchSocketUrl(this.runId));
      ...
      if (this.manualClose || this.subscribers.size === 0) return;
      ...
      ws.onclose = (event) => {
        ...
        if (event.code === 1008 || event.code === 4001) {
          ...
          this.cachedSocketUrlIndex =
            (attemptedSocketUrlIndex + 1) % this.cachedSocketUrls.length;
        }
        this.emit({
          type: "socket",
          status: "disconnected",
...
export function subscribeToExecutionRun(
  runId: string,
  subscriber: ExecutionSocketSubscriber,
): () => void {
  const normalizedRunId = runId.trim();
  if (!normalizedRunId) {
