  ...
  user: {
    changeEmail: {
      ...
      sendChangeEmailVerification: async ({ user, newEmail }) => {
        if (process.env.NODE_ENV !== "production") {
          console.info(
            `Email change verification requested for ${user.email} -> ${newEmail}`,
          );
        }
      },
    },
  ...
  secondaryStorage: useRedisSecondaryStorage
    ? {
        get: async (key: string) => {
          const { redis, withRedisFallback } =
            await import("@repo/shared/redis");
          return withRedisFallback(
            "auth:secondary-storage:get",
            async () => redis.get(key),
            async () => null,
          );
        },
