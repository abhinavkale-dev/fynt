...
function extractCredentialFields(data: unknown): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(objectValue)) {
    if (!/credentialid$/i.test(key)) continue;
...
function getTemplateBindingEntries(
  template: WorkflowTemplate,
): TemplateBindingEntry[] {
...
export function bindCredentialsToTemplate(
  template: WorkflowTemplate,
  ...
  const updatedNodes = template.nodes.map((node) => {
    ...
    if (bindingEntry) {
      ...
      if (!resolvedCredentialId) {
        const isPlatformUnique =
          (platformBindingCount.get(bindingEntry.binding.platform) ?? 0) === 1;
        if (isPlatformUnique) {
          resolvedCredentialId = platformFallbackMap.get(
            bindingEntry.binding.platform,
          );
        }
      }
    }
    ...
    const existingCredentialId =
      typeof existingCredentialIdRaw === "string" &&
      existingCredentialIdRaw.trim().length > 0
    ...
    const nodeFieldRequirements =
      template.fieldRequirements?.filter(
        (req) => req.nodeId === node.id && req.required,
      ) || [];
...
export function instantiateTemplateGraphPreservingCredentials(
  template: WorkflowTemplate,
  currentNodes: Node[],
  credentialBindings?: CredentialBinding[],
  additionalFields?: Record<string, Record<string, string>>,
  ...
  const resetGraph = instantiateTemplateGraph(
    template,
    credentialBindings,
    additionalFields,
    options,
