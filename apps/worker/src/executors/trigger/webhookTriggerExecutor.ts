...
export async function executeWebhookTriggerNode(
  ...
  const metadata = (nodeRun.workflowRun.metadata ?? {}) as Record<
    string,
    unknown
  >;
  const source =
    typeof metadata.source === "string" ? metadata.source : "webhook";
  ...
  if (executionMode === "strict_template_v1" && source !== "webhook") {
    throw new Error(
      "Webhook trigger requires an inbound webhook request. Use the configured webhook URL instead of manual execution.",
    );
  }
  return {
    source,
