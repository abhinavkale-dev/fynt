...
export async function executeCronTriggerNode(
  ...
  if (executionMode === "strict_template_v1" && source !== "cron") {
    throw new Error(
      "Cron trigger requires scheduler execution. Configure this workflow as cron or use a manual trigger for manual runs.",
    );
  }
  return {
    source,
