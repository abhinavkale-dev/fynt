...
class NotionRequestError extends Error {
  ...
  constructor(operation: string, status: number, detail: string, hint: string) {
    super(
      `Notion API error during ${operation} (${status}): ${detail}${hint ? `\nHint: ${hint}` : ""}`,
    );
...
function getNotionErrorHint(status: number): string {
  switch (status) {
    ...
    default:
      ...
      if (status >= 500) return "Server error from Notion API.";
      return "";
  }
}
...
function parseJsonTemplate(
  ...
  expected: "object" | "array" | "object_or_array" = "object",
): Record<string, unknown> | unknown[] {
  const parsed = parseTemplateWithMetadata(
    template,
  ...
  try {
    ...
    throw new Error(
      `${label} must resolve to valid JSON. ${message} ` +
...
function parseNotionId(
  ...
  if (!normalized) {
    throw new Error(
      `Notion ${fieldName} must be a valid Notion ID or Notion URL.`,
    );
  }
  if (isClearlyInvalidNotionId(normalized)) {
    throw new Error(
      `Notion ${fieldName} is invalid (${parsed}). Use a real Notion page/data-source/database ID or URL and ensure the integration is shared to that resource.`,
    );
  }
  return normalized;
}
function parseNotionTargetId(
  dataSourceIdValue: string | undefined,
  databaseIdValue: string | undefined,
  runMetadata: Record<string, NodeExecutionOutput>,
  operation: "create" | "get_many",
): {
  id: string;
  source: "dataSourceId" | "databaseId";
} {
  const dataSourceId = normalizeNotionIdentifier(
    parseOptionalTemplate(dataSourceIdValue, runMetadata),
  );
  if (dataSourceId) {
    if (isClearlyInvalidNotionId(dataSourceId)) {
      throw new Error(
        `Notion ${operation} dataSourceId is invalid (${dataSourceId}). Provide a real Notion data source ID/URL and share it with your integration.`,
      );
    }
    return { id: dataSourceId, source: "dataSourceId" };
  }
  const databaseId = normalizeNotionIdentifier(
    parseOptionalTemplate(databaseIdValue, runMetadata),
  );
  if (databaseId) {
    if (isClearlyInvalidNotionId(databaseId)) {
      throw new Error(
        `Notion ${operation} databaseId is invalid (${databaseId}). Provide a real Notion database ID/URL and share it with your integration.`,
      );
    }
    return { id: databaseId, source: "databaseId" };
  }
  throw new Error(
    `Notion ${operation} operation requires dataSourceId (recommended) or databaseId (legacy). You can paste either an ID or a Notion URL.`,
  );
}
function parseOptionalTemplate(
  value: string | undefined,
  runMetadata: Record<string, NodeExecutionOutput>,
): string | undefined {
  if (!value?.trim()) {
...
async function looksLikeNotionPageIdTarget(
  targetId: string,
  accessToken: string,
): Promise<boolean> {
...
async function looksLikeLegacyDatabaseTarget(
  targetId: string,
  accessToken: string,
): Promise<boolean> {
...
async function maybeRewriteTargetNotFoundError(
  error: unknown,
  targetId: string,
  accessToken: string,
): Promise<unknown> {
  ...
  if (
    error.status !== 404 ||
    !error.detail.toLowerCase().includes("object_not_found")
  ) {
    return error;
  }
  ...
  const hint =
    "The provided target ID resolves to a page, not a database/data source. " +
...
function shouldTryLegacyCreateFallback(
  _idSource: "dataSourceId" | "databaseId",
  error: unknown,
): boolean {
  if (!(error instanceof NotionRequestError)) {
    return false;
  ...
  return (
    detail.includes("data_source_id") ||
...
function shouldTryLegacyQueryFallback(
  _idSource: "dataSourceId" | "databaseId",
  error: unknown,
): boolean {
  if (!(error instanceof NotionRequestError)) {
...
async function executeCreate(
  ...
  const target = parseNotionTargetId(
    data.dataSourceId,
    data.databaseId,
    runMetadata,
    "create",
  ...
  const properties = parseJsonTemplate(
  ...
  try {
    ...
    let shouldUseLegacyFallback = shouldTryLegacyCreateFallback(
      target.source,
      error,
    );
    if (!shouldUseLegacyFallback) {
      const isLegacyDatabase = await looksLikeLegacyDatabaseTarget(
        target.id,
        accessToken,
      );
    ...
    if (!shouldUseLegacyFallback) {
      throw await maybeRewriteTargetNotFoundError(
        error,
        target.id,
        accessToken,
      );
...
async function executeGet(
  ...
  const parsedBody = await callNotion(
    endpoint,
    {
      method: "GET",
      headers: getNotionHeaders(accessToken),
...
async function executeGetMany(
  ...
  try {
    ...
    let shouldUseLegacyFallback = shouldTryLegacyQueryFallback(
      target.source,
      error,
    );
    if (!shouldUseLegacyFallback) {
...
async function executeUpdate(
  ...
  const body: Record<string, unknown> = {};
  if (data.propertiesTemplate?.trim()) {
    body.properties = parseJsonTemplate(
      data.propertiesTemplate,
      "propertiesTemplate",
      runMetadata,
      "object",
...
export async function executeNotionNode(
  data: NotionNodeData,
  runMetadata: Record<string, NodeExecutionOutput>,
  _executionMode: ExecutionMode = "legacy",
  ownerUserId: string,
): Promise<NodeExecutionOutput> {
  ...
  if (credential.platform !== "notion") {
    throw new Error(
      `Credential platform mismatch: notionNode requires notion credential, got ${credential.platform}`,
    );
  }
