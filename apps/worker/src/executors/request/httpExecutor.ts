...
function getHttpErrorHint(status: number, method: string): string {
  switch (status) {
    case 400:
      return "Check the request body format and required fields";
    case 401:
      return "Check authentication — the endpoint may require an API key or token in headers";
    case 403:
      return "Access denied — verify your API key permissions or token scope";
    case 404:
      return "Verify the URL path is correct";
    case 405:
      return `This endpoint does not accept ${method} requests — try a different HTTP method`;
    case 408:
      return "The server timed out — try increasing the timeout value";
    case 413:
      return "Request body is too large — reduce the payload size";
    case 429:
      return "Rate limited — wait and retry, or reduce request frequency";
    case 500:
      return "The remote server encountered an internal error";
    case 502:
      return "Bad gateway — the remote server's upstream is unavailable";
    case 503:
      return "The remote service is temporarily unavailable — retry later";
    default:
...
export async function executeHTTPNode(
  ...
  if (Array.isArray(headers)) {
    headers.forEach((h) => {
      if (h.key && h.value) {
        headersObj[h.key] = parseTemplate(
          h.value,
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      if (
        ...
        return {
          ...
          ...(!("statusCode" in objectPayload)
            ? { statusCode: responseMeta.statusCode }
            : {}),
          ...(!("status" in objectPayload)
            ? { status: responseMeta.status }
            : {}),
          ...(!("statusText" in objectPayload)
            ? { statusText: responseMeta.statusText }
            : {}),
          ...(!("headers" in objectPayload)
