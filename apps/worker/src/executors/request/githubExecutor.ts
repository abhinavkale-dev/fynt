...
function getGitHubErrorHint(status: number): string {
  switch (status) {
    ...
    default:
      ...
      if (status >= 500) return "Server error from GitHub API.";
      return "";
  }
}
...
function parseLabels(
  labelsTemplate: string | undefined,
  runMetadata: Record<string, NodeExecutionOutput>,
): string[] | undefined {
  ...
  try {
    ...
    throw new Error(
      `labelsTemplate must resolve to valid JSON array. ${message} ` +
...
async function callGitHub(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `GitHub API error during ${operation} (${response.status}): ${detail}${hint ? `\nHint: ${hint}` : ""}`,
    );
  }
  return parsedBody;
}
function getGitHubHeaders(
  accessToken: string,
  withJsonContentType: boolean = false,
): HeadersInit {
  return {
...
async function executeCreateIssue(
  ...
  const parsedBody = await callGitHub(
    endpoint,
    {
      method: "POST",
...
async function executeCreateComment(
  ...
  const endpoint =
    `${GITHUB_BASE_URL}/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}` +
...
export async function executeGitHubNode(
  ...
  if (credential.platform !== "github") {
    throw new Error(
      `Credential platform mismatch: githubNode requires github credential, got ${credential.platform}`,
    );
  }
