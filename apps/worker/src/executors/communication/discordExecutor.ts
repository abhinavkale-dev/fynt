...
function parseWebhookIdentity(webhookUrl: string): {
  ...
  try {
    ...
    if (parts[0] !== "api" || parts[1] !== "webhooks") return null;
    const webhookId = parts[2];
...
export async function executeDiscordNode(
  ...
  const parsedResult = parseTemplateWithMetadata(
    message,
    runMetadata as Record<string, string>,
  );
  const parsedMessage = parsedResult.output;
  const parsedUsername = parseTemplate(
    username || "Fynt Bot",
    runMetadata as Record<string, string>,
  );
  const missingVarList =
    parsedResult.missingVars?.join(", ") || "unknown variables";
  if (
    executionMode === "strict_template_v1" &&
    parsedResult.missingVars?.length
  ) {
    throw new Error(
      `Message template uses undefined variables: ${missingVarList}. ` +
  ...
  if (!formattedMessage) {
    if (executionMode === "strict_template_v1") {
      throw new Error(
        "Discord node not configured - message is empty after formatting",
      );
    }
    return {
      success: true,
  ...
  if (!isDiscordWebhookUrl(discordWebhook)) {
    throw new Error(
      `Invalid Discord webhook URL in credential: ${redactWebhookUrl(discordWebhook)}`,
    );
  }
  try {
  ...
  const discordChannelId =
    typeof responseJson?.channel_id === "string"
      ? responseJson.channel_id
      : undefined;
  const discordGuildId =
    typeof responseJson?.guild_id === "string"
      ? responseJson.guild_id
      : undefined;
  const deliveredContent =
    typeof responseJson?.content === "string"
  ...
  if (webhookIdentity) {
    try {
      const webhookInfoRes = await fetch(
        `${webhookIdentity.origin}/api/webhooks/${webhookIdentity.webhookId}/${webhookIdentity.webhookToken}`,
      );
      if (webhookInfoRes.ok) {
        const webhookInfo = (await webhookInfoRes.json()) as Record<
          string,
          unknown
        >;
        webhookResolvedChannelId =
          typeof webhookInfo.channel_id === "string"
            ? webhookInfo.channel_id
            : undefined;
        webhookResolvedGuildId =
          typeof webhookInfo.guild_id === "string"
    ...
    if (discordMessageId) {
      try {
        const verifyMessageRes = await fetch(
          `${webhookIdentity.origin}/api/webhooks/${webhookIdentity.webhookId}/${webhookIdentity.webhookToken}/messages/${discordMessageId}`,
        );
  ...
  const postedToDifferentChannel = !!(
    webhookResolvedChannelId &&
    discordChannelId &&
    webhookResolvedChannelId !== discordChannelId
  );
  return {
    success: true,
    ...
    ...(truncated
      ? { truncated: true, originalLength: formattedMessage.length }
      : {}),
  };
}
