...
import {
  resolveCredential,
  resolveSingleCredentialByPlatform,
} from "../../engine/credentialResolver.js";
...
export async function executeSlackNode(
  ...
  if (!formattedMessage) {
    if (executionMode === "strict_template_v1") {
      throw new Error(
        "Slack node not configured - message is empty after formatting",
      );
    }
    return {
      success: true,
  ...
  if (typeof credentialId === "string" && credentialId.trim().length > 0) {
    ...
    try {
      const { platform, keys } = await resolveCredential(
        normalizedCredentialId,
        ownerUserId,
      );
      if (platform.toLowerCase() !== "slack") {
        throw new Error(
          `Credential ${normalizedCredentialId} is not a Slack credential`,
        );
      }
      ...
      credentialResolutionError =
        error instanceof Error
          ? error
          : new Error("Failed to resolve Slack credential");
      // Recover stale template credential IDs when there is only one Slack credential for the user.
      const fallbackCredential = await resolveSingleCredentialByPlatform(
        "slack",
        ownerUserId,
      );
      if (fallbackCredential) {
        resolvedWebhookUrl = normalizeSlackWebhookUrl(
          fallbackCredential.keys.webhookUrl,
        );
      }
    }
  }
  ...
  if (!resolvedWebhookUrl) {
    if (credentialResolutionError) {
      throw new Error(
        `${credentialResolutionError.message}. This workflow likely has a stale Slack credential. Re-select the Slack credential on the node.`,
      );
    }
