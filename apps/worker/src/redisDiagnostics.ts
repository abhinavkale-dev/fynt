...
function appendCommonNotes(
  summary: RedisConfigurationSummary,
  hasBothConfiguredUrls: boolean,
): RedisConfigurationSummary {
  if (hasBothConfiguredUrls) {
    summary.notes.push(
      "Both REDIS_URL and UPSTASH_REDIS_URL are set. REDIS_URL takes precedence.",
    );
  }
  return summary;
}
...
function isConnectionMessage(message: string): boolean {
  ...
  return (
    lowered.includes("connection is closed") ||
...
export function isLikelyRedisConnectionError(error: unknown): boolean {
  ...
  if (
    nestedErrors.some((nestedError) =>
      isLikelyRedisConnectionError(nestedError),
    )
  ) {
    return true;
  }
...
export function logRedisTroubleshooting(prefix: string, error: unknown): void {
  ...
  if (isRuntimeModeDisabledError(error)) {
    printTroubleshootingStep(
      prefix,
      "FYNT_RUNTIME_MODE is web-only. Set FYNT_RUNTIME_MODE=full before starting the worker.",
    );
    return;
  }
  ...
  if (summary.isUrlInvalid) {
    printTroubleshootingStep(
      prefix,
      `Invalid Redis URL configuration detected (${summary.displayTarget}).`,
    );
    printTroubleshootingStep(
      prefix,
      "Use REDIS_URL=redis://host:port or REDIS_URL=rediss://host:port for managed Redis/Upstash.",
    );
    return;
  }
  ...
  printTroubleshootingStep(
    prefix,
    `Redis connection failed for ${summary.displayTarget}.`,
  );

  if (summary.hasImplicitLocalFallback) {
    printTroubleshootingStep(
      prefix,
      "No Redis env vars were set, so worker defaulted to localhost:6379.",
    );
    printTroubleshootingStep(
      prefix,
      "Start Redis locally or set REDIS_URL / UPSTASH_REDIS_URL in the repo root .env.",
    );
    return;
  }
  ...
  if (summary.source === "REDIS_HOST_PORT") {
    printTroubleshootingStep(
      prefix,
      "Check REDIS_HOST/REDIS_PORT values and confirm Redis is reachable from this machine.",
    );
    return;
  }
  ...
  printTroubleshootingStep(
    prefix,
    `Check ${summary.source} and confirm the URL is reachable.`,
  );
  printTroubleshootingStep(
    prefix,
    "If using Upstash, prefer rediss:// URL and verify the token and hostname are complete.",
  );
}
